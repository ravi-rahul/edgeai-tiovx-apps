...
int32_t parse_input_node(InputInfo *input_info, const YAML::Node &input_node)
{
    ...
    input_info->viss_dcc_path[0] = '\0';
    input_info->ldc_dcc_path[0] = '\0';
    ...
    /* Parse viss-dcc-path. */
    if (input_node["viss-dcc-path"])
    {
        sprintf(input_info->viss_dcc_path,
                input_node["viss-dcc-path"].as<std::string>().data());
    }

    /* Parse ldc-dcc-path. */
    if (input_node["ldc-dcc-path"])
    {
        sprintf(input_info->ldc_dcc_path,
                input_node["ldc-dcc-path"].as<std::string>().data());
    }
...
int32_t parse_model_node(ModelInfo *model_info, const YAML::Node &model_node)
{
    ...
    if(pre_proc_node["data_layout"].as<std::string>() == "NCHW")
    {
        model_info->pre_proc_info.channel_order = 0;
    }
    else
    {
        model_info->pre_proc_info.channel_order = 1;
    }
...
void dump_data(FlowInfo flow_infos[], uint32_t num_flows)
{
    ...
    for(i = 0; i < num_flows; i++)
    {
        ...
        TIOVX_APPS_PRINTF("\tviss_dcc_path: %s\n",flow_infos[i].input_info.viss_dcc_path);
        TIOVX_APPS_PRINTF("\tldc_dcc_path: %s\n",flow_infos[i].input_info.ldc_dcc_path);
